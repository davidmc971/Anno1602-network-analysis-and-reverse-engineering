...
pub fn parse_message(message: &mut Message) {
    ...
    if maybe_dplay_signature == [112, 108, 97, 121] {
        ...
        let version = version_and_command_cursor
            .read_u16::<LittleEndian>()
            .unwrap();
        println!("DPLAY, size: {}, token: 0x{:x}, SockAddr: [AF: 0x{:x}, port: {}, ip_addr: 0x{:x}, signature: play, version: {:?}, command: {:?}]", size_u20, token_u12, sock_addr_in_address_family, sock_addr_in_port, sock_addr_in_ip_address, version, command);
